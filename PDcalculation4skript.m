function [bfout,Vout] = PDcalculation4skript(PMT, PA, cpth, bf,Vout)
...
%   cpth    path to common folder
%   bf      bias field (optional)
%==========================================================================
    ...
    if(~exist('bf','var'))
       ...
       Atmp = zeros(dm);
        ...
        Vtmp = my_write_vol_nii(WMmask,VMTforA,'WMmask_');
        copyfile(Vtmp.fname,cpth);        
        [pth, fname, ext] = spm_fileparts(Vtmp.fname);
        Pout = spm_select('FPList', cpth,['^' fname ext]);
        Vout = spm_vol(Pout);
    ...
    A_WM = WMmask.*Y;
    scalA = nanmean(A_WM(A_WM~=0 & abs(A_WM)~=Inf));
    stdA = std(A_WM(A_WM~=0 & abs(A_WM)~=Inf), [], 1);
    Y = Y/scalA*69;
    sprintf('mean White Matter intensity: %04d', scalA)
    sprintf('SD White Matter intensity %04d', stdA)
    Y(Y>200 ) = 0;
    % MFC: Estimating Error for data set to catch bias field issues:
    errorEstimate = stdA./scalA;
    Vsave = spm_vol(PA);
    [pA, fA, eA] = spm_fileparts(PA);
